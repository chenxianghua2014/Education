				...
				<tbody>
					...
					<tr>
						...
						<td><input type="hidden" id="subjectType" name="subjectType"
							value="${subject.subjectType}" />多选题(六个选项)</td>
					</tr>
					...
					<tr>
						<th><span class="warning">*</span>E</th>
						<td><script id="subjectE" name="subjectE" type="text/plain"
								style="width:100%;height:60px;"></script></td>
					</tr>
					<tr>
						<th><span class="warning">*</span>F</th>
						<td><script id="subjectF" name="subjectF" type="text/plain"
								style="width:100%;height:60px;"></script></td>
					</tr>
					<tr>
						<th><span class="warning">*</span>选择题答案</th>
						<td>A <input type="checkbox" id="subjectAnswer_A"
							name="subjectAnswer" value="A"
							<c:out value="${fn:indexOf(subject.subjectAnswer, 'A') > -1 ? 'checked':''}"/> />
							B <input type="checkbox" id="subjectAnswer_B"
							name="subjectAnswer" value="B"
							<c:out value="${fn:indexOf(subject.subjectAnswer, 'B') > -1 ? 'checked':''}"/> />
							C <input type="checkbox" id="subjectAnswer_C"
							name="subjectAnswer" value="C"
							<c:out value="${fn:indexOf(subject.subjectAnswer, 'C') > -1 ? 'checked':''}"/> />
							D <input type="checkbox" id="subjectAnswer_D"
							name="subjectAnswer" value="D"
							<c:out value="${fn:indexOf(subject.subjectAnswer, 'D') > -1 ? 'checked':''}"/> />
							E <input type="checkbox" id="subjectAnswer_E"
							name="subjectAnswer" value="E"
							<c:out value="${fn:indexOf(subject.subjectAnswer, 'E') > -1 ? 'checked':''}"/> />
							F <input type="checkbox" id="subjectAnswer_F"
							name="subjectAnswer" value="F"
							<c:out value="${fn:indexOf(subject.subjectAnswer, 'F') > -1 ? 'checked':''}"/> />
						</td>
					</tr>
					<tr>
						<th></th>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		editorE = UE.getEditor('subjectE', {
			toolbars : [ toobars ],
			maximumWords : 200
		});
		editorE.addListener("ready", function() {
			editorE.setContent('${subject.subjectE }');
		});
		editorF = UE.getEditor('subjectF', {
			toolbars : [ toobars ],
			maximumWords : 200
		});
		editorF.addListener("ready", function() {
			editorF.setContent('${subject.subjectF }');
		});
		...
		$('form').validator({
			...
			valid : function(form) {
				...
				if (editorE.getContent().length == 0) {
					alert("请填写E选项内容!");
					editorE.focus();
					return;
				}
				if (editorF.getContent().length == 0) {
					alert("请填写F选项内容!");
					editorF.focus();
					return;
				}
