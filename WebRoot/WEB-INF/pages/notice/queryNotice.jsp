...
<script type="text/javascript">
	function Addnew(sjid,cj) {
		window.location = "skipID?id=0&noticeSort="+sjid+"&noticeRank="+cj;
	}

	function collectionNoticeAll(cj,noticeSort) {
		window.location = "collectionNoticeAll?cj="+cj+"&page=1&noticeSort="+noticeSort;
	}
	
	function stickNotice(sjid,noticeSort) {
		window.location = "stickNotice?id="+sjid+"&mark=1&noticeSort="+noticeSort;
	}
...
function editNew(newsId,noticeSort) {
	window.location = "skipID?id=" + newsId+"&mark=1&noticeSort="+noticeSort;
}
...
<body>
	<div class="title">当前位置:<c:if test="${notice.noticeSort==2}">党校</c:if>信息管理><c:if test="${notice.noticeSort==2}">党校</c:if>通知管理</div>
	<div class="editBlock">
		
		<table>
			<tbody>
				<tr>
					<td height="32" colspan="4" class="subtitle"><c:if test="${notice.noticeSort==2}">党校</c:if>通知管理</td>
				</tr>
				<tr>
					<td style="text-align: left"><input name="button" type="button"
						class="inputButton" value="添加"  onclick="Addnew('${notice.noticeSort}','${cj}')"/>
						<input name="button" type="button"
						class="inputButton" value="切换收藏"  onclick="collectionNoticeAll('${cj}','${notice.noticeSort}')"/>
					</td>
						
					<td style="text-align: right;">
					<form action="NoticeAll?mark=1&noticeSort=${notice.noticeSort}&page=1&cj=${cj}" method="post"  >
					发布级别:<select
						id="noticeRank" name="noticeRank">
							<option value=0 >全部</option>
							<option value=1 ${notice.noticeRank==1?"selected":""}>一级</option>
							<option value=2 ${notice.noticeRank==2?"selected":""}>二级</option>
							<option value=3 ${notice.noticeRank==3?"selected":""}>三级</option>
					</select>
					<input type="hidden" id="organizationId" name="organizationId" value="${notice.organizationId}" />
					&nbsp;
					            发布单位:<input type="text" name="noticeCatalog" class="inputText" id="noticeCatalog" value="${notice.noticeCatalog}" /> &nbsp;
						发布时间:<input type="text" name="noticeTime" class="inputText" id="noticeTime"  value="${notice.noticeTime}" onclick='WdatePicker()'/> &nbsp;
	...
	<div class="dataGrid">
		<table>
			<caption><c:if test="${notice.noticeSort==2}">党校</c:if>通知查询结果</caption>
			<thead>
				<tr>
				    <!-- <th>序      号</th> -->
				    <th>通知编号</th>
			...
			<tbody id="">
				...
				<%-- 	<td>${status.count}</td> --%>
					...
					<td class='alignCenter'><input name='button'type='button' onclick="stickNotice('${maps.noticeId}','${maps.noticeSort}')" class='inputButton' value='置顶' /></td>
					</c:when>
					<c:otherwise>
					...
					<td class='alignCenter'><input name='button'type='button' onclick="collection('${maps.noticeId}')" class='inputButton' value='收藏' /></td>
