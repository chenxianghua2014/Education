...
<script type="text/javascript">
	function Addnew(sjid,cj) {
		window.location = "programmeskipID?id=0&pageNewsId="+sjid+"&programmeRank="+cj;
	}
	
		function collectionNoticeAll(cj) {
		window.location = "collectionProgrammeAll?cj="+cj+"&page=1";
	}
	
	function stickNotice(sjid) {
		window.location = "stickProgramme?id="+sjid+"&mark=0";
	}
...
function editNew(newsId) {
	window.location = "programmeskipID?id=" + newsId+"&mark=0";
}
...
<body>
	<div class="title">当前位置:<c:if test="${programme.programmeSort==2}">党校</c:if>信息管理><c:if test="${programme.programmeSort==2}">党校</c:if>计划收藏</div>
	<div class="editBlock">
		
		<table>
			<tbody>
				<tr>
					<td height="32" colspan="4" class="subtitle"><c:if test="${programme.programmeSort==2}">党校</c:if>计划收藏</td>
				</tr>
			</tbody>
		</table>
	...
	<div class="dataGrid">
		<table>
			<caption><c:if test="${programme.programmeSort==2}">党校</c:if>计划收藏查询结果</caption>
			<thead>
				<tr>
				    <!-- <th>序      号</th> -->
				    <th>计划编号</th>
			...
			<tbody id="">
				...
				<%-- 	<td>${status.count}</td> --%>
					...
					<td class='alignCenter'><input name='button'type='button' onclick="collection('${maps.programmeId}')" class='inputButton' value='取消' /></td>
					...
					<td class='alignCenter'><input name='button'type='button' onclick="editNew('${maps.programmeId}')" class='inputButton' value='编辑' /></td>
