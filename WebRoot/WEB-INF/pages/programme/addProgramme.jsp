...
<body>
	<div class="title">当前位置:<c:if test="${programmeSort==2}">党校</c:if>计划管理>编辑<c:if test="${programmeSort==2}">党校</c:if>计划</div>
	<div class="editBlock">
		<form action="createProgramme" method="post" class="registerform" id="fff"  name="fff">
			<input type="hidden" id="programmeSort" name="programmeSort" value="${programmeSort}" /> 
			<table>
				<tr>
					<td colspan="2" class="subtitle">编辑<c:if test="${programmeSort==2}">党校</c:if>计划信息</td>
				</tr>
				<tbody>
				<tr >
						<th width="150">置顶选择:</th>
						<td>
						<input type="radio" name="programmeTop"  id="programmeTop"  value="0" checked="checked" onclick="radioType()"/> 否 
						<input type="radio"  id="programmeTop"  name="programmeTop" value="1" onclick="radioType()" /> 是 
						</td>
					</tr>
					<tr style="display: none" id = "toptype">
						<th width="150">置顶截止日期:</th>
						<td><input type="text" name="programmeToptime" class="inputText" id="programmeToptime"  value="" onclick='WdatePicker()'/></td>
					</tr>
					
					<tr >
						<th width="150">名称:</th>
						<td><input id="programmeName" name="programmeName" type="text" class="inputText" style="width:300px;"  value="" datatype="*" errormsg="不能为空！"/></td>
					</tr>
					<tr>
						<th width="150">制作人:</th>
						<td><input id="programmeProducer" name="programmeProducer" type="text" class="inputText" style="width:300px;" value=""  datatype="*6-16" errormsg="请填写6到16位任意字符！"/></td>
					</tr>
					...
					<tr>
						...
						<td>
						<!-- <textarea id="editor_id" name="programmeContent" style="width:600px;height:320px;"  datatype="*" errormsg="不能为空！"></textarea> -->
                         <textarea id="TxtFile" name="programmeContent"   style="width:700px;height:320px;" datatype="*" errormsg="不能为空！"></textarea>
                          </td>
					</tr>
				</tbody>
				<tr>
