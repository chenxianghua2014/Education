...
function editNew(newsId) {
	window.location = "AccountskipID?id=" + newsId;
}
...
<body>
	<div class="title">当前位置:系统管理>后台管理</div>
	<div class="editBlock">
		
		<table>
			<tbody>
				<tr>
					<td height="32" colspan="4" class="subtitle">后台管理</td>
				</tr>
				<tr>
					<td style="text-align: left"><input name="button" type="button"
						class="inputButton" value="添加"  onclick="Addnew()"/></td>
						
					<td style="text-align: right;">
					<form action="accountAll?mark=1&page=1" method="post"  >
					&nbsp;
						账户:<input type="text" name="accountLoginid" class="inputText" id="accountLoginid"  value="${account.accountLoginid}" /> &nbsp;
		...
		<table>
			...
			<tbody id="">
				...
				<tr>
					...
					<td class='alignCenter'><input name='button'type='button' onclick="editNew('${maps.accountId}')" class='inputButton' value='编辑' /></td>
					<c:if test="${maps.accountId != sessionScope.sessionUser.accountId}">
