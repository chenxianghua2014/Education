...
function classOnes(classresourceId,cj,upname) {
	/* alert(classresourceId)
	alert(cj) */
	window.location = "queryShift?classresourceId=" + classresourceId+"&mark=1&page=1&cj="+cj+"&classlb="+upname;
}
...
<body>
	...
	<div class="editBlock">
		<form action="queryClassCatalog?organizationId=${pageId}&mark=1&page=1&type=1&cj=${cj}&classresourceUpname=${classCatalog.classresourceUpname}" method="post" name="userForm" id="userForm">
		<table>
			<tbody>
				<tr>
					<td height="32" colspan="4" class="subtitle">培训班管理</td>
				</tr>
				 <tr>
					<td style="text-align: left">
					
					<c:if test="${pageId==sessionScope.sessionUser.accountCatalog}">
					<input name="button" type="button"
						class="inputButton" value="添加"  onclick="Addclass('${classCatalog.organizationId}','${cj}',${classCatalog.classresourceUpname})"/>
					</c:if> 
					
					<c:if test="${pageId!=sessionScope.sessionUser.accountCatalog}">
					<input name="button" type="button"
						class="inputButton" value="添加" style="color:#AFB8C0" />
					</c:if> 
					...
					&nbsp;
					          培训班名称:<input type="text" name="classresourceName" class="inputText" id="classresourceName" value="${classCatalog.classresourceName}"/> &nbsp;
		...
		<table>
			...
			<thead>
				<tr>
				    ...
				    <th>培训班名称</th>
					<th>培训班级别</th>
					 <th width="60" class="alignCenter">详情</th>
			...
			<tbody id="">
				...
				<tr>
					<td>${status.count}</td> 
					<td>${maps.classresourceName }</td>
					<td>${maps.classresourceRank }</td>
					<td class='alignCenter'><input name='button'type='button' onclick="classOnes('${maps.classresourceId}','${cj}','${classCatalog.classresourceUpname}')" class='inputButton' value='详情' /></td>
					
					<c:if test="${maps.organizationId==sessionScope.sessionUser.accountCatalog}">
					<td class='alignCenter'><input name='button'type='button' onclick="editClass('${maps.classresourceId}','${cj}')" class='inputButton' value='编辑' /></td>
