...
function getRootPath(){
    ...
    ch = document.getElementById('purl').value;
    ch1 = document.getElementById('purl1').value;
    return (localhostPaht + projectName);
};
$(window).load(function() {
	$('#newsImage').uploadify({
        'swf': getRootPath() + '/js/uploadify/uploadify.swf?ver='+ Math.random(),
        'uploader':'upplansort?paurl='+ch+'&paurl1='+ch1,// uploadify.swf 文件的相对路径   //文件保存路径
...
//        			 ' - 服务器端消息: ' + data+
//        			 ' - 是否上传成功: ' + response);
        	 document.getElementById('courseImage').value=data;
             $('#myTargetDivPic').html("<img src=\"./" + data + "\" style=\"border:1px solid #000;height: 185px;width:330px\"/>"); 
        }
