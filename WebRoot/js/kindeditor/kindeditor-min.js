/*******************************************************************************
 * KindEditor - WYSIWYG HTML Editor for Internet Copyright (C) 2006-2010 Longhao
 * Luo
 * @author Roddy <luolonghao@gmail.com>
 * @modifyauthor: Elkan <584205853@qq.com>
...
(function(undefined) {
    ...
    KE.setting = {
        ...
        statusbarHeight: 11,
        items: ["source", "preview", "|", "fullscreen", "undo", "redo", "print", "cut", "copy", "paste", "plainpaste", "wordpaste", "|", "justifyleft", "justifycenter", "justifyright", "justifyfull", "insertorderedlist", "insertunorderedlist", "indent", "outdent", "subscript", "superscript", "date", "time", "|", "selectall", "-", "title", "fontname", "fontsize", "|", "textcolor", "bgcolor", "bold", "italic", "underline", "strikethrough", "removeformat", "|", "image", "accessory", "flash", "media", "advtable", "hr", "link", "unlink", "specialchar", "|", "about"],
    ...
    a.plugin.accessory = {
        ...
        exec: function(g) {
            ...
            if (e == 1) {
                ...
                //-----------------------------------           
            var uploadIframe = a.$('uploadIframe', d);   //需要有个iframe
            var onloadFunc = function() {
    				a.event.remove(uploadIframe, 'load', onloadFunc);
    				a.util.hideLoadingPage(g);
    				var uploadDoc = a.util.getIframeDoc(uploadIframe);
    				var data = '';
    				try {
    					data = a.util.parseJson(uploadDoc.body.innerHTML);
    				} catch(e) {
    					alert(a.lang.invalidJson);
    				}
    				if (typeof data === 'object' && 'error' in data) {
    					if (data.error === 0) {
    						var url = a.format.getUrl(data.url, 'absolute');
    						self.insert(g, url, data.title , data.ext);
    					} else {
    						alert(data.message);
    						return false;
    					}
                ...
                a.event.add(uploadIframe, 'load', onloadFunc);
       ...
       //---------------------------------
        ...
        insert: function(j, d, i, f) {
            ...
            var e = '<img class="filetype" src="' + c + '" title="' + h + '">&nbsp;';
            e += '<a href="' + d + '" title="' + i + '">';
            if (i) {
                e += i
            }
