...
function beforeRemove(treeId, treeNode) {
	...
	if(confirm("确认删除【" + treeNode.name + "】 吗？")){
		ajaxPost("delTreeNode", {syllabusId:treeNode.id}, function(msg){
			if(msg.status=='error'){
				alert(msg.head);
			}
			zNodes = [];
...
function beforeRename(treeId, treeNode, newName, isCancel) {
	...
	if (newName.length == 0) {
		...
		ajaxPost("updateTreeNode", {syllabusId:treeNode.id, syllabusName:newName}, function(msg){
			if(msg.status=='error'){
				alert(msg.head);
			}
			zNodes = [];
...
function addHoverDom(treeId, treeNode) {
	...
	if (btn) btn.bind("click", function(){
		...
		var syllabus = {syllabusName: dirname, syllabusUpperId: treeNode.id};
		
		ajaxPost("addTreeNode", syllabus, function(msg){
			if(msg.status=='error'){
				alert(msg.head);
			}else{
			zNodes = [];
...
function backData(msg){
	...
	for(var i=0; i<msg.msgBody.length; i++){
		zNodes.push({ id:msg.msgBody[i].syllabusId,pId:msg.msgBody[i].syllabusUpperId, name:msg.msgBody[i].syllabusName, open:true, 
			icon:"images/file.gif", iconOpen:"images/folder-closed.gif", iconClose:"images/folder-closed.gif",click:"direct2('" + msg.msgBody[i].syllabusId + "');"});
	}
	//console.log(msg);
...
function direct2(syllabusId){
	window.parent.document.getElementById('rightMain').src="Estimate?page=1&mark=1&syllabusId="+syllabusId;
}
